...
namespace eduJSON.Tests
{
    ...
    public class ParserTests
    {
        ...
        public void ParseTest()
        {
            ...
            Assert.AreEqual(1234, Parser.Parse(" 1234 "));
            Assert.AreEqual(1234, Parser.Parse(" +1234 "));
            Assert.AreEqual(-1234, Parser.Parse(" -1234 "));
            Assert.AreEqual(1.0870e-3, (double)Parser.Parse(" +1.0870e-3 "), 1e-10);
            Assert.AreEqual(-2e+31, (double)Parser.Parse(" -2e+31 "), 1e-10);
            Assert.AreEqual("This is a test.", Parser.Parse(" \"This is a test.\" "));
            ...
            Assert.AreEqual(3, obj_dict.Count);
            Assert.AreEqual(true, obj_dict["key1"]);
            CollectionAssert.AreEqual(new List<object>
                {
                    ...
                    4,
                    5
                }, (List<object>)obj_dict["key2"]);
            CollectionAssert.AreEqual(new Dictionary<string, object>
                {
                    { "k1", "test1" },
                    { "k2", "test2" }
                }, (Dictionary<string, object>)obj_dict["key3"]);
        }
    }
}